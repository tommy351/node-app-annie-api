...
['Reviews'].forEach(function(i){
  AppAnnie.prototype['product' + i] = function(){
    if (!this.options.vertical) throw new Error('options.vertical is required!');
    if (!this.options.market) throw new Error('options.market is required!');
    if (!this.options.asset) throw new Error('options.asset is required!');
    if (!this.options.productId) throw new Error('options.productId is required!');
    return new this.Query(apiBaseUrl + '/' + this.options.vertical + '/' +  this.options.market + '/' +
        this.options.asset + '/' + this.options.productId + '/' + i.toLowerCase());
  };
});
